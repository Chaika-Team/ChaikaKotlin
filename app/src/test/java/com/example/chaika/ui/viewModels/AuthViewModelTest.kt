...
class AuthViewModelTest {
    ...
    fun setUp() {
        ...
        // Создаем экземпляр ViewModel, передавая моки зависимостей
     ...
     */
    @Test
    fun `processDeepLink sets accessToken when use cases succeed`() = runTest {
        val testIntent = Intent("action_deeplink")
        val expectedToken = "token123"
        val dummyConductor = ConductorDomain(
            id = 1,
            name = "John",
            familyName = "Doe",
            givenName = "John",
        ...
        whenever(handleAuthorizationResponseUseCase.invoke(testIntent)).thenReturn(expectedToken)
        whenever(authorizeAndSaveConductorUseCase.invoke(expectedToken)).thenReturn(dummyConductor)
     ...
     * Описание:
     *  - Тест для AuthViewModel.
     *  - Если handleAuthorizationResponseUseCase выбрасывает исключение, то error должен быть обновлён.
     */
    @Test
    fun `processDeepLink sets error when use case throws exception`() = runTest {
        val testIntent = Intent("action_deeplink")
        val exceptionMessage = "Invalid token"
        whenever(handleAuthorizationResponseUseCase.invoke(testIntent)).thenThrow(
            RuntimeException(
                exceptionMessage
            )
        )
